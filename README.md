...
### Audioburst API key
The library requires an application key which can be obtained via [Audioburst Publishers](https://publishers.audioburst.com/). Optionally you can also get the experience ID. It is a unique identifier for the customized playlist topics chosen during the setup process in Audioburst Publishers.
...
A valid application key is required to initialize AudioburstPlayer. Player itself can be configured in two ways: via [Audioburst Publishers](https://publishers.audioburst.com/) and obtaining experience ID or by passing custom configuration.
